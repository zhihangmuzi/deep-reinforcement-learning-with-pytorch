import gym
from DuelingDQN.Agent import DuelDQN
import torch
...
dueldqn = DuelDQN(state_dim=state_dim, action_dim=action_dim, device=device)
flag = False
...
for episode in range(400):
    ...
    while True:
        ...
        action = dueldqn.choose_action(state)
        ...
        dueldqn.store_transition(state, action, new_reward, state_, done)

        ep_r += new_reward
        if ep_r > 1200:
            torch.save(dueldqn.eval_net.state_dict(), 'eval.pth')
            flag = True
            done = True
        ...
        if done:
            ...
            print('episode: ', episode,
                  'ep_r: ', round(ep_r, 2),
                  'epsilon: ', round(dueldqn.epsilon, 2))
            break
