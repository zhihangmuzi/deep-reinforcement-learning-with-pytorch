...
class Douboe_DQN(object):
    ...
    def learn(self):
        ...
        q_eval_index = self.eval_net(Variable(b_s_)).detach().max(1)[1].reshape([-1, 1])
        q_next = self.target_net(Variable(b_s_)).gather(1, q_eval_index).detach()
